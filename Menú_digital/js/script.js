...
function iniciarMicSeleccion() {
    // Esta función se mantiene por si en el futuro se activa manualmente, pero ya no se llama automáticamente
    if (!micSeleccionActivo) return;
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) return;
    if (modoSeleccionRecognition) return;
...
function mostrarPantallaSeleccion() {
    ...
    if (welcomeEl) {
        welcomeEl.style.display = 'flex';
        // CORRECCIÓN: hablar sin iniciar micrófono automáticamente
        setTimeout(() => {
            hablar(
                "Bienvenido al menú digital accesible. " +
                "Opción uno: Sin barrera de comunicación. " +
...
function selectMode(mode) {
    ...
    if (mode === 'blind') {
        ...
        if (overlay) {
            overlay.style.display = 'flex';
            // CORRECCIÓN: hablar sin iniciar reconocimiento de voz
            setTimeout(() => {
                hablar(
                    "¿Cómo prefiere interactuar? " +
                    "Opción uno: Lector de pantalla del dispositivo. Usa el TTS de su teléfono. " +
...
// Mic para elegir entre voice/reader en blind-choice-overlay (ahora solo se activa si se llama manualmente)
let blindChoiceRec = null;
function iniciarMicElegirCiego() {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition || blindChoiceRec) return;
...
function mostrarPantallaSeleccion() {
    ...
    // Ocultar ambos overlays primero, luego mostrar solo el principal
...
function selectMode(mode) {
    ...
    if (mode === 'blind') {
        ...
        // Mostrar submenú de elección para ciegos
...
function aplicarModo(mode, blindChoice = null) {
    ...
    // Si es modo ciego sin subopción elegida, mostrar el submenú de elección primero
    ...
    // Guardar en sessionStorage solo cuando el modo está completamente definido
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Siempre mostrar la pantalla de selección al cargar la página.

    // El modo de accesibilidad no se persiste entre sesiones — solo la orden.
